...
import { HAlign, ParseSpecialChars, TextRenderer, VAlign } from "./TextRenderer";
...
export class DxfScene {
	...
	*_DecomposeEllipse(entity, blockCtx) {
		...
		const isClosed =
			(entity.endAngle ?? null) === null || Math.abs(endAngle - startAngle - 2 * Math.PI) < 1e-6;
	...
	*_DecomposeHatch(entity, blockCtx) {
		if (entity.isSolid) {
			// console.log("current solid hatch entity is hatch = ", JSON.stringify(entity));

			const color = this._GetEntityColor(entity, blockCtx);
			const layer = this._GetEntityLayer(entity, blockCtx);
			for (let loopIndex = 0; loopIndex < entity.boundaryLoops.length; loopIndex++) {
				...
				if (vertices.length >= 2) {
					// console.log("adding entity to yield in hatch detection = ", entity.type);
					yield new Entity({
						entityType: entity.type,
						handle: entity.handle,
						ownerHandle: entity.ownerHandle,
			...
			if (style != HatchStyle.ODD_PARITY && style != HatchStyle.THROUGH_ENTIRE_AREA) {
				// console.log("_DecomposeHatch not HatchStyle.ODD_PARITY  ", entity, " blockCtx ", blockCtx);
				return;
			}

			const boundaryLoops = this._GetHatchBoundaryLoops(entity);
			// console.log("current boundary loops are = ", JSON.stringify(boundaryLoops));
			...
			// console.log("_DecomposeHatch at end seedPoint = ", seedPoints, " blockCtx ", blockCtx);
		}
	}
			...
			if (loop.type & 2) {
				...
				for (let vtxIdx = 0; vtxIdx < loop.polyline.vertices.length; vtxIdx++) {
					...
					if ((vtx.bulge ?? 0) == 0) {
						...
						const prevVtx =
							loop.polyline.vertices[vtxIdx == 0 ? loop.polyline.vertices.length - 1 : vtxIdx - 1];
						if ((prevVtx.bulge ?? 0) == 0) {
							/* Start vertex is not produced by _GenerateBulgeVertices(). */
							vertices.push(new Vector2(vtx.x, vtx.y));
						}
						const nextVtx =
							loop.polyline.vertices[vtxIdx == loop.polyline.vertices.length - 1 ? 0 : vtxIdx + 1];
	...
	_FlattenBatch(entity, blockBatch, layerName, blockColor, blockLineType, transform) {
